...
namespace ThaiTuanERP2025.Api.Controllers.FileStorage
{
	...
	public class FilesController : ControllerBase
	{
		...
		public async Task<ActionResult<object>> Upload(IFormFile file, CancellationToken cancellationToken) {
			...
			var bucket = _cfg["Minio:Bucket"]!;
			await _storage.EnsureBucketAsync(bucket, cancellationToken);
			...
			await _storage.UploadAsync(bucket, objectKey, stream, file.ContentType, cancellationToken);

			var result = new { bucket, objectKey, fileName = file.FileName, size = file.Length };
		...
		public async Task<ActionResult<ApiResponse<string>>> GetDownloadUrl([FromQuery] string bucket, [FromQuery] string objectKey, CancellationToken cancellationToken)
		{
			...
			var url = await _storage.GetPresignedGetUrlAsync(bucket, objectKey, TimeSpan.FromSeconds(seconds), cancellationToken);
