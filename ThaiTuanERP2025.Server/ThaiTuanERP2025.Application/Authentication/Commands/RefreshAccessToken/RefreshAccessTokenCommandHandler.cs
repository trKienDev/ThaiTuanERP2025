...
namespace ThaiTuanERP2025.Application.Authentication.Commands.RefreshAccessToken
{
	public sealed class RefreshAccessTokenCommandHandler : IRequestHandler<RefreshAccessTokenCommand, LoginResponseDto>
	{
		...
		public async Task<LoginResponseDto> Handle(RefreshAccessTokenCommand request, CancellationToken ct)
		{
			...
			var tokenEntity = await _refreshRepo.GetByTokenAsync(request.RefreshToken, ct)
			...
			var user = await _userReadRepository.GetWithRolesAndPermissionsByIdAsync(tokenEntity.UserId, ct)
			...
			await _refreshRepo.AddAsync(newTokenEntity, ct);
